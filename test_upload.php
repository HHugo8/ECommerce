...
{
	require('connect.php');
	$result = $bdd->prepare('INSERT INTO items (id_items, nom, category, link, extension,ISBN, description, artist, price, size, upload_date, isApproved) VALUES(0 , :nom, :category, :link, :extension, :isbn, :description, :artist, :price, :size, :upload_date, 0)');
	$result->execute(array('nom' => $nom , 'category' => $category, 'link' => $chemin , 'extension' => $extension,'isbn' => $isbn, 'description' => $description , 'artist' => $artist, 'price' => $prix, 'size' => $size, 'upload_date' => $upload_date ));
};
function random($car) 
{
	$string = "";
